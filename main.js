...
app.controller('itemsDropDownController', function($scope, $timeout) {
    $scope.dropdawnBtn = 'Select home';
    // document.getElementById("editSaveBtn").disabled = true;
    $scope.homeNames_id = getHomesNames_id(home);
    $scope.saveDisable = true;
    $scope.clickFunc = function(text, id){
        $scope.removeAll();
        $scope.dropdawnBtn = text;
        $scope.homeSelect = text;
        $scope.searching_id = id;
        $scope.oldHomeName = text;
    };
    $scope.saveFunc = function (txt, searh_id) {
        let homeNumber;
        for (let i = 0; i < ($scope.homeNames_id.length); i++) {
            if ($scope.homeNames_id[i]._id === searh_id) {homeNumber = i}
        }
        if ( !txt) {
            $scope.error = true;
            return;
        ...
        $scope.saveDisable = true;
        $timeout($scope.removeAll, 3000);
        // $timeout.cancel(timer);
    };
    $scope.updCheck = function () {
        $scope.removeAll();
        $scope.saveDisable = false;
        if ( $scope.homeSelect === $scope.oldHomeName) {
            $scope.saveDisable = true;
        }
    };
...
function getHomesNames_id (arr) {
    let resultArr = [];
    let resultObj = {};
    for (let i = 0; i < arr.length; i++) {
        resultObj._id = arr[i]._id;
        resultObj.homeName = arr[i].homeName;
